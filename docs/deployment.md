...
# Isi SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, LEADERBOARD_PASSCODE, SESSION_SECRET.
npm install
npm run dev
```
...
3. Tambahkan empat environment variable di **Settings → Environment Variables** untuk Production, Preview, dan Development (lihat bagian 2 di atas).
...
Tidak ada admin panel khusus — semua dikelola lewat **Supabase Table
Editor** (bagian 26 pada spesifikasi):
...
- Jangan pernah memakai prefix `NEXT_PUBLIC_` untuk `SUPABASE_SERVICE_ROLE_KEY`, `LEADERBOARD_PASSCODE`, atau `SESSION_SECRET`.
